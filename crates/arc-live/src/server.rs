...
function draw(s){
 const x=s.overlay||s.stats||{},active=preset(presetOverride||x.preset),lang=language(languageOverride||x.language),t=lang==='ru'?{title:{account:'Статистика аккаунта',session:'С запуска ARC Live',outcome:'Победы | Поражения'},account:['Ноки игроков','Урон рейдерам','Вынесено'],session:['Ноки за стрим','Успешные выходы','Баланс'],outcome:['Вышел живым','Погиб'],mode:['Демо','Эфир']}:{title:{account:'Account totals',session:'Current stream',outcome:'Win | Lose'},account:['Player knocks','Raider damage','Extracted value'],session:['Stream knocks','Successful exits','Balance'],outcome:['Extracted alive','Knocked out'],mode:['Demo','Live']};
 formatter=new Intl.NumberFormat(lang==='ru'?'ru-RU':'en-US');for(const name of ['account','session','outcome'])el(name).classList.toggle('active',name===active);setText('title',t.title[active]);
 const accountLabels=(lang==='ru'?x.widget_account_labels_ru:x.widget_account_labels_en)||t.account,sessionLabels=(lang==='ru'?x.widget_session_labels_ru:x.widget_session_labels_en)||t.session,outcomeLabels=(lang==='ru'?x.widget_outcome_labels_ru:x.widget_outcome_labels_en)||t.outcome;
