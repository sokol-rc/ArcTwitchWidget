...
impl ArcLiveApp {
    ...
    fn start_new_stream(&mut self) {
        let (baseline, overlay, snapshot) = {
            ...
            state.overlay.session_raw_totals.clear();
            self.widget_config.apply(&mut state.overlay);
            state.record("info", "A new stream session was started by the user");
        ...
        self.record_user_event("success", "Начат новый стрим — счётчики обнулены");
    ...
    fn apply_probe(&mut self, payload: ProbePayload) {
        ...
        self.widget_config.apply(&mut overlay);
        {
            ...
            overlay.preset = match state.overlay.preset.as_str() {
                "session" | "outcome" => state.overlay.preset.clone(),
                _ => "account".to_owned(),
            };
    ...
    fn ui(&mut self, root: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(root, |ui| {
            ...
            if selected_preset != snapshot.overlay.preset {
                let updated = {
                    let mut state = self.state.write().expect("state poisoned");
                    state.overlay.preset = selected_preset;
                    state.record("info", "OBS widget preset changed");
                    state.clone()
                };
                self.server.notify(&updated);
            }
            ...
            match self.page {
                AppPage::Home => {
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        if ui
                            .add_enabled(can_reset, egui::Button::new("Начать новый стрим"))
                            .clicked()
                        {
                    ...
                    if self.confirm_new_stream {
                        egui::Frame::group(ui.style())
                            ...
                            .show(ui, |ui| {
                                ui.label("Обнулить ноки, выходы и баланс текущего стрима?");
                                ui.horizontal(|ui| {
                                    if ui.button("Да, начать заново").clicked() {
                ...
                AppPage::Widget => {
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        ui.selectable_value(&mut edited.preset, "session".to_owned(), "С запуска");
