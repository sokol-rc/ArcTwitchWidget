...
pub struct WidgetConfig {
    ...
    pub pve: [WidgetMetric; 2],
    #[serde(default = "default_pvp")]
    pub pvp: [WidgetMetric; 2],
}
...
impl WidgetConfig {
    pub fn load_or_create(path: &Path) -> Result<Self> {
        ...
        if let (Some(document), Some(defaults)) = (document.as_object_mut(), defaults.as_object()) {
            for name in ["pve", "pvp"] {
                if !document.contains_key(name) {
                    document.insert(name.to_owned(), defaults[name].clone());
                    migrated = true;
                }
            }
        }
    ...
    pub fn apply(&self, stats: &mut OverlayStats) {
        ...
        stats.widget_pve = self.pve.each_ref().map(|metric| metric.resolve(stats));
        stats.widget_pvp = self.pvp.each_ref().map(|metric| metric.resolve(stats));
        ...
        stats.widget_outcome_labels_en = self
            ...
            .each_ref()
            .map(|metric| metric.label_en.clone());
        stats.widget_pve_labels_ru = self.pve.each_ref().map(|metric| metric.label_ru.clone());
        stats.widget_pve_labels_en = self.pve.each_ref().map(|metric| metric.label_en.clone());
        stats.widget_pvp_labels_ru = self.pvp.each_ref().map(|metric| metric.label_ru.clone());
        stats.widget_pvp_labels_en = self.pvp.each_ref().map(|metric| metric.label_en.clone());
    }
}
...
fn default_pve() -> [WidgetMetric; 2] {
    [
        metric("session.loot_value", "Вынесено за стрим", "Stream loot"),
        WidgetMetric {
...
mod tests {
    ...
    #[test]
    fn pve_and_pvp_presets_use_current_stream_deltas() {
        let mut stats = OverlayStats {
            session_loot_value: 125_000,
            ..Default::default()
        };
        stats.session_raw_totals.insert("event.101".into(), 42_000);
        stats
            .session_raw_totals
            .insert("event.101.target.995408715".into(), 9_000);
        stats
            .session_raw_totals
            .insert("event.101.target.200993951".into(), 1_500);
        stats
            .session_raw_totals
            .insert("event.200.target.995408715".into(), 4);

        WidgetConfig::default().apply(&mut stats);

        assert_eq!(stats.widget_pve, [125_000, 31_500]);
        assert_eq!(stats.widget_pvp, [4, 10_500]);
    }

    #[test]
    fn old_widget_config_gets_new_preset_defaults() {
        let old = r#"{
            "schema_version": 1,
            "account": [{"source":"account.downs","label_ru":"a","label_en":"a"},{"source":"account.downs","label_ru":"b","label_en":"b"},{"source":"account.downs","label_ru":"c","label_en":"c"}],
            "session": [{"source":"session.downs","label_ru":"a","label_en":"a"},{"source":"session.downs","label_ru":"b","label_en":"b"},{"source":"session.downs","label_ru":"c","label_en":"c"}],
            "outcome": [{"source":"outcome.wins","label_ru":"a","label_en":"a"},{"source":"outcome.losses","label_ru":"b","label_en":"b"}]
        }"#;
        let config: WidgetConfig = serde_json::from_str(old).unwrap();
        config.validate().unwrap();
        assert_eq!(config.pve[0].source, "session.loot_value");
        assert_eq!(config.pvp[0].source, "session.event.200.target.995408715");
    }

    #[test]
    fn loading_old_file_persists_new_preset_sections() {
        let root = std::env::temp_dir().join(format!(
            "arc-live-widget-config-{:016x}",
            rand::random::<u64>()
        ));
        std::fs::create_dir_all(&root).unwrap();
        let path = root.join("widget-config.json");
        let mut old = serde_json::to_value(WidgetConfig::default()).unwrap();
        old.as_object_mut().unwrap().remove("pve");
        old.as_object_mut().unwrap().remove("pvp");
        std::fs::write(&path, serde_json::to_vec_pretty(&old).unwrap()).unwrap();

        let loaded = WidgetConfig::load_or_create(&path).unwrap();
        let saved: serde_json::Value =
            serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();

        assert_eq!(loaded.pve[0].source, "session.loot_value");
        assert!(saved.get("pve").is_some());
        assert!(saved.get("pvp").is_some());
    ...
    fn empty_profile_never_receives_preview_values() {
        ...
        assert_eq!(stats.widget_pve, [0, 0]);
        assert_eq!(stats.widget_pvp, [0, 0]);
    }
}
