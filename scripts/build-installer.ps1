...
try {
    ...
    $wixPath = if (Test-Path -LiteralPath $localWix) {
        $localWix
    } else {
        $wixCommand = Get-Command wix.exe -ErrorAction SilentlyContinue
        if ($wixCommand) {
            if ($wixCommand.Path) { $wixCommand.Path } else { $wixCommand.Source }
        }
    }
    if (-not $wixPath) {
    ...
    & $wixPath build 'installer\Product.wxs' -arch x64 -d "StageDir=$stageDirectory" -d "ProductVersion=$version" -o $msiPath
    ...
    & $wixPath build 'installer\Bundle.wxs' -arch x64 -ext $balExtension.FullName -d "ProjectDir=$projectDirectory" -d "MsiPath=$msiPath" -d "ProductVersion=$version" -o $setupPath
