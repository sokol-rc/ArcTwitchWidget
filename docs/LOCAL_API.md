...
Виджет нарисован под размер 700 × 80 и сам масштабируется под размер Browser
Source, отрисовывая текст в конечном разрешении. Поэтому крупный виджет
получают, задавая размер источника (например 1400 × 160), а не растягивая
источник в сцене - растягивание увеличивает уже готовую картинку и мылит
шрифты. Масштаб можно задать вручную через `scale=0.25..8`, а прежнее
поведение без подгонки вернуть через `fit=off`.
