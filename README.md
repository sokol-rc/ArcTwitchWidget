...
The widget has three presets selectable in ARC Live: lifetime account totals,
statistics accumulated during the current stream, and a compact Win/Lose
counter that falls back to the API's daily scope until the current session has a
result. An OBS source can override the selected preset with `?preset=1`,
`?preset=2`, or `?preset=3`. The app also switches the widget between Russian
...
for the current day. `Начать новый стрим` is the only action that resets stream
counters, and it requires confirmation.
